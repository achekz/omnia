...
import { Users, Briefcase, Activity, AlertTriangle, CheckCircle2, Minus } from "lucide-react";
  ...
  return (
    <ModuleLayout>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <StatCard
          title="Active Alerts"
          value={stats?.anomaliesDetected || 3}
          icon={<AlertTriangle className="w-8 h-8 text-rose-400" />}
          delay={0.4}
        />
      </div>

      {/* Row 2: Charts & Performers */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        ...
        <div className="glass-panel rounded-2xl p-6 border border-white/5 flex flex-col">
          ...
          <div className="space-y-4 flex-1">
            {!teamLoading && teamMembers?.slice(0, 5).map((teamData: any, i: number) => {
              const member = teamData.member;
              return (
                <div key={member._id || member.id} className="flex items-center justify-between p-3 rounded-xl hover:bg-white/5 transition-colors">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-secondary overflow-hidden border border-white/10">
                      <img src={`https://images.unsplash.com/photo-${1535713875002 + i}-d1d0cf377fde?w=100&h=100&fit=crop`} alt={member.name} className="w-full h-full object-cover" />
                    </div>
                    <div>
              ...
              {teamMembers?.map((teamData: any) => {
                ...
                return (
                  <tr key={member._id || member.id} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={cn("px-2.5 py-1 text-xs font-semibold rounded-full border", getRiskColor('low'))}>
                        LOW
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      {member.isActive !== false ? (
                        ...
                        <span className="flex items-center gap-1.5 text-xs text-muted-foreground"><Minus className="w-3.5 h-3.5" /> Inactive</span>
                      )}
                    </td>
                  </tr>
                );
              })}
              {/* Fallback if no members returned yet */}
              {!teamMembers?.length && !teamLoading && (
                <tr>
                  <td colSpan={5} className="px-6 py-8 text-center text-muted-foreground">No team members found.</td>
                </tr>
              )}
            </tbody>
          </table>
