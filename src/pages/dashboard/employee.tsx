...
import { CheckSquare, AlertCircle, Flame, Plus } from "lucide-react";
import { cn } from "@/lib/utils";
import { useGetTasks, useUpdateTask, useGetDashboardStats, useMlInsights } from "@/lib/api-client";
...
export default function EmployeeDashboard() {
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    const draggedTask = tasks.find(t => (t._id || t.id) === draggableId);
    ...
    const index = newTasks.findIndex(t => (t._id || t.id) === (draggedTask._id || draggedTask.id));
    newTasks[index] = { ...draggedTask, status: newStatus };
    setTasks(newTasks);
    ...
    if (taskId) {
      updateTask({ id: taskId, data: { status: newStatus } }, {
        onError: () => refetch() // Revert on error
      });
    }
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          ...
          <DragDropContext onDragEnd={onDragEnd}>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {columns.map(col => {
                ...
                return (
                  <div key={col.id} className="glass-panel rounded-xl border border-white/5 flex flex-col h-[500px]">
                      ...
                      {(provided, snapshot) => (
                        ...
                        >
                          {columnTasks.map((task, index) => (
                            <Draggable key={task._id || task.id} draggableId={(task._id || task.id)!.toString()} index={index}>
                              {(provided, snapshot) => (
                                <div
                                  ref={provided.innerRef}
                                  ...
                                  className={cn(
                                    ...
                                    snapshot.isDragging ? "border-primary glow-shadow scale-105" : "border-white/10 hover:border-white/20"
                                  )}
                                >
                                  <div className="flex justify-between items-start mb-2">
                                    <span className={cn(
                                      "text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded",
                                      task.priority === 'high' || task.priority === 'critical' ? "bg-rose-500/20 text-rose-400" :
                                  ...
                                  {task.dueDate && (
                                    <p className="text-xs text-muted-foreground flex items-center gap-1">
                                      <AlertCircle className="w-3 h-3" /> {new Date(task.dueDate).toLocaleDateString()}
                                    </p>
                                  )}
                                </div>
                              )}
        ...
        <div>
          ...
          <div className="space-y-4">
            {insights?.latestRecommendation?.recommendations?.map((rec: string, i: number) => (
              <MLInsightCard
                key={i} 
                type="recommendation"
                prediction={rec}
