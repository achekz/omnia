...
const defaultStats: AccountantStats = {
  totalClients: 0,
  revenue: 0,
  invoices: 0,
  expenses: 0,
...
export default function AccountantDashboard() {
  const { user } = useAuth();
  const [stats, setStats] = useState<AccountantStats>(defaultStats);
  ...
  const StatCard = ({ title, value, icon: Icon, color }: StatCardProps) => (
    ...
    >
      ...
      <div>
        <p className="text-sm font-medium text-gray-600 dark:text-gray-400 uppercase tracking-wider mb-2">{title}</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="glass-panel p-8 rounded-3xl">
            ...
            <div className="space-y-4">
              {[1, 2, 3, 4].map((index) => (
                <div key={index} className="flex items-center justify-between p-4 bg-white/50 dark:bg-gray-800/50 rounded-xl hover:shadow-md transition-shadow">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-semibold text-gray-900 dark:text-white">Invoice #{1000 + index}</p>
          ...
          <div className="glass-panel p-8 rounded-3xl">
            ...
            <div className="h-64 bg-white/50 dark:bg-gray-800/50 rounded-2xl p-6">
              <div className="h-full flex items-center justify-center text-gray-500">Revenue chart will appear here (Chart.js integration ready)</div>
            </div>
          </div>
        </div>
      </div>
