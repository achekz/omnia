...
const notificationPreferencesList: Array<{
            ...
            <nav className="flex flex-row md:flex-col gap-1 overflow-x-auto">
              <button
                ...
                className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-colors whitespace-nowrap ${
                  activeTab === "profile"
                    ? "bg-white dark:bg-gray-700 text-purple-700 dark:text-purple-300 shadow-sm border border-gray-200/50 dark:border-gray-600/50"
          ...
          <div className="flex-1 p-6 sm:p-8">
            {activeTab === "profile" && (
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      ...
                      <input type="email" value={user.email} disabled className="w-full pl-10 pr-4 py-2 bg-gray-100 border border-gray-200 rounded-lg text-gray-500 focus:outline-none cursor-not-allowed" />
                    </div>
                    <button onClick={() => setShowChangeEmail((current) => !current)} className="text-sm text-purple-600 font-medium hover:text-purple-700">
                  ...
                  <div className="space-y-4 p-4 rounded-lg bg-purple-50 border border-purple-200">
                    {!showEmailVerification ? (
                      ...
                      <>
                        ...
                        <button onClick={handleVerifyEmailCode} className="w-full px-4 py-2 bg-green-600 hover:bg-green-700 text-white font-medium rounded-lg transition-colors">
                          Verify & Update Email
                        </button>
                      </>
                    )}
                ...
                <div className="pt-4 flex justify-end">
                  <button onClick={handleSaveProfile} className="px-5 py-2.5 bg-purple-600 text-white font-medium rounded-lg hover:bg-purple-700 transition-colors shadow-sm shadow-purple-500/20">
                    Save Changes
                  </button>
                </div>
              </div>
            ...
            {activeTab === "security" && (
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="pt-2">
                  <button onClick={handleChangePassword} className="px-5 py-2.5 bg-gray-900 text-white font-medium rounded-lg hover:bg-gray-800 transition-colors">
                    Update Password
                  </button>
                </div>
              </div>
            ...
            {activeTab === "notifications" && (
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="space-y-4">
                  {notificationPreferencesList.map((preference) => (
                    <div key={preference.key} className="flex items-start gap-4 p-4 rounded-xl border border-gray-100 hover:border-purple-100 hover:bg-purple-50/30 transition-colors">
                      <div className="h-5 flex items-center mt-0.5">
                        <input
                          type="checkbox"
                          checked={notifications[preference.key]}
                          onChange={(event) =>
                            setNotifications((prev) => ({ ...prev, [preference.key]: event.target.checked }))
                          }
                          className="w-4 h-4 text-purple-600 rounded border-gray-300 focus:ring-purple-500"
                        />
                      </div>
                      <div>
                        <p className="text-sm font-semibold text-gray-900">{preference.title}</p>
                        <p className="text-sm text-gray-500 mt-0.5">{preference.desc}</p>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="pt-4">
                  <button onClick={handleSaveNotifications} className="px-5 py-2.5 bg-purple-600 text-white font-medium rounded-lg hover:bg-purple-700 transition-colors shadow-sm shadow-purple-500/20">
                    Save Preferences
                  </button>
                </div>
              </div>
