...
def presence_anomaly():
    ...
    repeated_absences = max("".join("A" if status == "absent" else "P" for status in statuses).count("AAA"), 0)

    if not delays:
        return jsonify({"is_anomaly": bool(repeated_absences), "anomaly_score": 0.5 if repeated_absences else 0.0, "signals": []})
