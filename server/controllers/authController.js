...
export const register = asyncHandler(async (req, res) => {
  ...
  // Map profileType → role
  const roleMap = {
    company: 'company_admin',
    cabinet: 'cabinet_admin',
    employee: 'employee',
    student: 'student',
  };
  const role = roleMap[profileType];
  if (!role) throw new ApiError(400, 'Invalid profileType');
  ...
  const user = await User.create({ name, email, password, role, profileType, tenantId });
